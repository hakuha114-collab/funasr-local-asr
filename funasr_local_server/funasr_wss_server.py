...
parser.add_argument(
    ...
    help="offline ASR model from modelscope (standard paraformer-zh, lighter than contextual)",
)
...
parser.add_argument(
    ...
    action="store_true",
    default=False,
    help="Enable speaker verification (SV). Off by default: the WS SV returns "
         "'unknown' unless a speaker_db.json is enrolled, so it is useless for "
         "real diarization (use the diarize_server on :10098 for true clustering).",
)
...
# sv (optional: only when --enable_sv; WS SV is speaker VERIFICATION, not
# real diarization, and returns 'unknown' without an enrolled speaker_db.json)
if args.enable_sv:
    model_sv = AutoModel(
    ...
    model_sv = None
    print("SV disabled (--enable_sv not set); speaker will show as 'unknown' on WS.")
...
async def async_asr(websocket, audio_in: bytes):
    ...
    # 2) 声纹识别（阻塞，线程池执行）；SV 未启用时直接跳过
    spk_name = "unknown"
    best_score = 0.0
    if model_sv is not None and len(audio_in) > 0:
        try:
    ...
    if len(text) > 0:
        ...
        try:
            ...
            print("[async_asr] client closed before sending final result:", e)
    else:
        message = {
            "mode": mode,
            "spk_name": spk_name,
        ...
        try:
            ...
            print("[async_asr] client closed before sending empty final result:", e)
