...
def _clear():
    # 跨平台清屏：Windows 用 cls，其余用 clear（原 os.system('clear') 在 Windows 会报错）
...
parser.add_argument("--speaker_diarization",
                    type=int,
                    default=0,
                    help="1 to enable speaker diarization (cam++), 0 to disable")
parser.add_argument("--speaker_num",
                    type=int,
                    default=0,
                    help="expected number of speakers (0 = auto)")
...
async def record_microphone():
    ...
    message = json.dumps({"mode": args.mode, "chunk_size": args.chunk_size, "chunk_interval": args.chunk_interval,
                          ...
                          "speaker_diarization": args.speaker_diarization, "speaker_num": args.speaker_num})
    #voices.put(message)
...
async def message(id):
    ...
    try:
       while True:
            ...
            spk_tag = "[spk{}] ".format(spk) if spk is not None else ""
            ...
            if meg["mode"] == "online":
                ...
                _clear()
                print("\rpid" + str(id) + ": " + spk_tag + text_print)
                ...
                # text_print = text_print[-args.words_max_print:]
                # _clear()
                print("\rpid" + str(id) + ": " + wav_name + ": " + spk_tag + text_print)
...
async def ws_client(id, chunk_begin, chunk_size):
  ...
  for i in range(chunk_begin,chunk_begin+chunk_size):
    ...
    async with websockets.connect(uri, subprotocols=[websockets.Subprotocol("binary")], ping_interval=None, ssl=ssl_context) as websocket:
