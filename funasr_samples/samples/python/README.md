...
--speaker_diarization [`1` to enable speaker diarization (cam++), `0` to disable]
--speaker_num [expected number of speakers, `0` for auto]

#### Speaker diarization (按声纹区分说话人)
服务端需带 `--spk-dir damo/speech_campplus_sv_zh-cn_16k-common-onnx` 启动（见 启动脚本.txt / docker-compose.yml）。
客户端开启说话人分离并解析 `spk` 字段:
```shell
python funasr_wss_client.py --host "127.0.0.1" --port 10096 --mode 2pass \
  --chunk_size "8,8,4" --audio_in "./meeting.wav" --speaker_diarization 1 --output_dir "./results"
```
> 注意: 容器内端口 10095，宿主机映射为 10096，连接宿主机请用 10096。
> 若在线服务未返回 spk 字段，建议改用仓库内 `diarize.py`（Python funasr + cam++，分离效果更可靠）。

## conda 环境（离线 diarize.py / Python API 路线）
与"以前在 conda 里跑 funasr"一致。需要 funasr Python 包（已锁定 1.3.30），见仓库根目录 `environment.yml`。
```shell
conda env create -f ../../../../environment.yml
conda activate funasr
python diarize.py --audio_in meeting.wav --output_dir ./results
```
> 纯 CPU 可跑 Paraformer + cam++；有 NVIDIA GPU 时加 `--device cuda` 更快。
> 该环境只服务于离线 Python 脚本，与上面的 Docker WebSocket 服务互不依赖。
```
